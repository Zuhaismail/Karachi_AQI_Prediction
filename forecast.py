...
# Forecast Next 3 Days (combined for all models)
df = pd.read_csv("karachi_aqi_features.csv", parse_dates=["timestamp"])
df.sort_values("timestamp", inplace=True)

# Force forecast to start from next midnight
last_time = df.iloc[-1]["timestamp"]
next_day_midnight = (last_time + timedelta(days=1)).replace(
    hour=0, minute=0, second=0, microsecond=0
)

# Generate 73 timestamps = midnight + 72 hours ahead
timestamps = [next_day_midnight + timedelta(hours=i) for i in range(73)]
...
for name in models.keys():
    ...
    temp_time = next_day_midnight

    # Generate predictions for 73 hours
